...
confluence_print/
  ...
  templates/
    index.html                 — Web GUI template (single-page app)
  static/
    pinklab_logo_text.png      — Logo symlink for web preview
  ...
  .gitignore                   — Excludes token file, output/, uploads/, __pycache__/
...
│  │  - Page URL input                                 │  │
│  │  - Logo upload / reset                            │  │
│  │  - Output dir + filename config                   │  │
│  │  - Progress log (polling /api/status)             │  │
│  └──────────────────────┬────────────────────────────┘  │
└─────────────────────────┼───────────────────────────────┘
                          │ REST API
┌─────────────────────────┼───────────────────────────────┐
│  app.py (Flask Server)  │                               │
│  ├─ GET  /api/defaults    → default output dir           │
│  ├─ POST /api/export      → start background thread     │
│  ├─ GET  /api/status/id   → progress + result + paths   │
│  ├─ GET  /api/files/path  → serve output file            │
│  ├─ POST /api/logo/upload → save to uploads/            │
│  └─ POST /api/logo/reset  → reset to default            │
│         │                                               │
│         ▼                                               │
│  confluence_export.py (Core Engine)                      │
│  ├─ load_token_file() → credentials                     │
│  ├─ extract_page_id() → page ID                         │
│  ├─ fetch_page() → Confluence REST API                  │
│  ├─ process_page_content() → processed HTML             │
│  ├─ build_html() + generate_pdf() → Document PDF        │
│  └─ build_presentation_html() + generate_pdf()          │
...
```
Input (Page URL — from Web GUI or CLI)
...
### Web GUI (app.py)
- `index()` — Serve main page (`templates/index.html`)
- `get_defaults()` — Return default output directory path
- `start_export()` — Accept export parameters (incl. per-file enable flags), launch background thread
- `job_status(job_id)` — Return progress log, completion status, and file paths
- `serve_output_file(filepath)` — Serve generated files for in-browser viewing
- `upload_logo()` — Save uploaded logo file to `uploads/`
- `reset_logo()` — Reset logo to default PinkLAB
